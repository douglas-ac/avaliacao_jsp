    ...
    <div class='container-fluid mt-3'>
        <div class='container p-0 bg-light mb-5 border border-5'>
            ...
            <div class="ps-3 pe-3 pt-2 pb-2">
                ...
                <div class='row align-items-center mb-3 bg-color-main text-white ps-3 pe-3 pt-1 pb-1 ms-5 me-5'>
                    <div class="col-2 d-flex flex-column align-items-center">
                        <h6><%out.write(c.getUsuario().getLogin());%></h6>
                        <span><%out.write(""+c.getTimestamp());%></span>
                    </div>
                    <div class='col-9 pt-3 pb-3 ps-5 pe-5'>
                        <p class='m-0'><%out.write(c.getTexto());%></p>
                    </div>
                    
                    <%if(perfil != null && perfil.equals(Perfil.MOD.name()) && !c.isStatus_aprovacao()){%>
                    <div class="col-1 d-flex flex-column align-items-center justify-content-center" action="">
                        <a onclick="return confirm('Confirmar aprovação do comentário?')" href=<%out.write("'aprovar_comentario.jsp?id_post="+id_post+"&id_comentario="+c.getId()+"'");%> class="btn btn-warning btn-sm mb-1">Aprovar</a>
                        <a onclick="return confirm('Confirmar exclusão do comentário?')" href=<%out.write("'excluir_comentario.jsp?id_post="+id_post+"&id_comentario="+c.getId()+"'");%> class="btn btn-danger btn-sm">Excluír</a>
                    </div>
                    <%}%>
                </div>
                <%}%>
